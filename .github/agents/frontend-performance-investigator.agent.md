...
model: GPT-5
tools: ['codebase', 'search', 'fetch', 'findTestFiles', 'problems', 'runCommands', 'runTasks', 'runTests', 'terminalLastCommand', 'terminalSelection', 'testFailure', 'openSimpleBrowser', 'chrome-devtools/*']
---
...
- Configure the Chrome DevTools MCP server under its documented `chrome-devtools` name; if using another server name, update the matching tool prefix above.
- If the required tools are unavailable, report that limitation before claiming runtime measurements.
