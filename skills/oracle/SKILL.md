...
- If the exported `OPENAI_API_KEY` is invalid and the user wants his personal OpenAI key, use `$keychain`. The Keychain item name is not recorded yet; confirm it with Ryan. Inject only into the single Oracle command; never print the key:
