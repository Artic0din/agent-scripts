...
- Add a `lint:boundaries` script covering all detected application and package source roots, for example `depcruise src` when both live under `src`, or `depcruise app packages` when they are siblings.
