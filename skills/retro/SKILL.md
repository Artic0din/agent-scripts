...
1. Read and apply [writing-for-agents](../writing-for-agents/SKILL.md) for the writing style guide using the host's file-reading tool.
...
- **Global AGENTS.md**: are there any steering instructions that should be moved to coding standards (or automated checks) instead? Before recommending a move, verify every applicable review entry point loads the destination file; otherwise keep the rules in the existing loaded instructions. _Use when_ the AGENTS.md file is particularly large - in the repo OR the user's global scope.
...
- `CODING_STANDARDS.md`: use this file for review-only standards only when the repository's review workflows explicitly load it.
  Its filename alone does not load it into a reviewer.
  Add **navigation pointers** to docs folders if the standards file gets more than 1,000 lines long.
