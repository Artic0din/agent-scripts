...
Find the existing verification skill under the repository's registered `.agents/skills`, `.claude/skills`, or `.cursor/skills` location.
...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (under the project's registered skill root). Several candidates → ask which one; none → stop and point at `/create-verification-skill` instead of inventing a target.
