...
1. **Always capture before state BEFORE making any changes** — if you forget, capture the baseline in a separate worktree
2. **Before/after pairs must use the same viewport width and crop** — otherwise the comparison is useless
3. **To get a "before" after you already changed code**: identify the correct baseline revision and run it in a separate temporary detached Git worktree, with its own app instance and port. Capture there, then stop only that instance and remove only the temporary worktree you created. Never overwrite files or the index in the working checkout to obtain a screenshot; preserve staged and unstaged changes throughout.
