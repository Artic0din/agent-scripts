---
name: skill-cleaner
description: "Codex/Claude skill audit: live budget, usage, duplicates, compact descriptions."
---
...
node --experimental-strip-types skills/skill-cleaner/scripts/skill-cleaner.ts --root ~/.claude/skills --no-logs
...
- `Unused candidates`: no recent user mention or actual `SKILL.md` read in recent Codex or Claude logs.
...
- Keep repo-local skills when they encode repo policy or live operations.
...
- It scans `~/.codex/history.jsonl` and recent `~/.codex/sessions/**/*.jsonl` by default. Add `--deep-logs` for archived sessions.
