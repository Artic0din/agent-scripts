...
Create `.github/instructions/migration.instructions.md` so Copilot can discover the migration instructions.
Set `applyTo` to valid glob patterns for the analyzed migration scope; replace the placeholder below with those patterns before saving.

\`\`\`markdown
---
applyTo: "[MATCHED_FILE_GLOBS]"
---
