...
- `MAC_RELEASE_OP_ACCOUNT` defaults to `my.1password.com`; `MAC_RELEASE_OP_VAULT`, `MAC_RELEASE_OP_TMUX_SESSION`, `MAC_RELEASE_OP_WAIT_SECONDS` are optional. Credential reads use the default tmux server and `op-work` session. Without a vault, service-account token env is unset for that single `op` read so the personal desktop account handles it.
